...
//开始页面
let startAlert = document.querySelector('#start')

// logo
let gameDesc = document.querySelector('.game-logo')
let footer = document.querySelector('#start footer')
...
let gameOverAlert = document.querySelector('#game-over')
//游戏结束
function gameOver(){
  clearInterval(timer)
...
let gameCompleteAlert = document.querySelector('#game-next')
// 游戏通关
function nextGame(){
  clearInterval(timer)
  clearInterval(updater)
  // config.status = 1;
  gameCompleteAlert.style.display = 'block'
}
...
nextBtn.onclick = function(){
  gameCompleteAlert.style.display = 'none'
  next()
}
